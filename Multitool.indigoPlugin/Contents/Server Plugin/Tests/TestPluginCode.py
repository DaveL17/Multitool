...
class TestPlugin(TestCase):
    ...
    def test_action_group_execution(self):
        """
        Test plugin Actions Groups

        When executed, Action Groups return `None`
        """
        LOGGER.warning("Test action group execution.")
        # Test the action to run a test
        # action = indigo.actionGroup.execute(ACTION_ID)
        # test_case.assertIsNone(action, "Error running action.")

        # Test the action to update test associated with a device
    ...
    def test_plugin_functions(self):
        ...
        LOGGER.warning("Test plugin functions")
        try:
            # ===================================== About Indigo =====================================
            test_case.assertTrue(self.about_indigo(no_log=False), "Method failed.")

            # ===================================== Battery Level Report =====================================
            test_case.assertTrue(self.battery_level_report(no_log=False), "Method failed.")
            ...
            for test_dict in dicts:
                test_case.assertTrue(self.color_picker(test_dict, no_log=False), "Method failed.")
            ...
            test_case.assertIsInstance(self.device_inventory(values_dict, no_log=False), dict, "Method failed.")

            # ===================================== Device Last Comm =====================================
            test_case.assertIsNone(self.device_last_successful_comm({'listOfDevices': 'indigo.relay'}, no_log=True), "Method failed.")

            # ===================================== Device Ping =====================================
            # TODO: Ping requires either Z-Wave or Insteon be enabled.

            # ===================================== Environment Path =====================================
            test_case.assertTrue(self.environment_path(no_log=False), "Method failed")

            # ===================================== Error Inventory =====================================
            for values_dict in [{'error_level': 'err'}, {'error_level': 'err_warn'}]:
                test_case.assertTrue(self.error_inventory(values_dict, no_log=False), "Method failed")

            # ===================================== Indigo Inventory =====================================
            test_case.assertTrue(self.indigo_inventory(no_log=False), "Method failed")
            ...
            # We do Network Quality Report above when testing devices.
            ...
            for payload in [{'classOfThing': 'actionGroups', 'thingToPrint': 460267384},
                ...
                # Print Object Dict
                test_case.assertTrue(self.results_output(payload, no_log=False), "Method failed.")

                # Print Object Dir
                test_case.assertTrue(self.object_directory(payload, no_log=False), "Method failed.")

                # Print Object Dependencies
                test_case.assertTrue(self.object_dependencies(payload, no_log=False), "Method failed.")
            ...
            # ===================================== Plugin Inventory =====================================
            test_case.assertTrue(self.installed_plugins(no_log=False), "Method failed.")

            # ===================================== Plugins Online =====================================
            test_case.assertTrue(self.running_plugins(no_log=False), "Method failed.")

            # ===================================== Remove Delayed Actions =====================================
            # Standard Indigo command; may not need testing.

            # ===================================== Scripts Embedded =====================================
            test_case.assertIsInstance(self.search_embedded_scripts({'search_string': ''}, no_log=False), tuple, "Method failed.")
            test_case.assertTrue(self.search_embedded_scripts({'search_string': ''}, no_log=False)[0], "Method failed.")
            test_case.assertIsInstance(self.search_embedded_scripts({'search_string': 'A'}, no_log=False), tuple, "Method failed.")
            test_case.assertTrue(self.search_embedded_scripts({'search_string': 'A'}, no_log=False)[0], "Method failed.")

            # ===================================== Scripts Linked =====================================
            values_dict = {}
            test_case.assertIsInstance(self.search_linked_scripts(values_dict, no_log=False), tuple, "Method failed.")
            test_case.assertTrue(self.search_linked_scripts(values_dict, no_log=False)[0], "Method failed.")

            # ===================================== Send Status Request =====================================
            # Standard Indigo command; may not need testing.

            # ===================================== Serial Ports =====================================
            test_case.assertTrue(self.get_serial_ports({'ignoreBluetooth': True}, no_log=False), "Method failed.")
